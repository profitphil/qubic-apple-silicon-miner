...
struct Miner
{
    ...
    // PATCH (qiner-metal): the 512MB random2 pool is owned by the host (one page-aligned
    // mmap allocation shared zero-copy with Metal). The reference no longer allocates its own.
    const unsigned char* pool = nullptr;

    // PATCH (qiner-metal): optional per-window trace for GPU-vs-CPU divergence isolation.
    // When non-null, score() writes the predicted trit of each window (255 on timeout).
    unsigned char* windowTrace = nullptr;

    bool initialize(const unsigned char* externalPool, const char* taskFilePath)
    {
        pool = externalPool;
    ...
    bool initializeFromMemory(const unsigned char* externalPool, const unsigned char* topoBlock, const unsigned char* dataBlock)
    {
        pool = externalPool;
    ...
    unsigned int score()
    {
        ...
        for (unsigned long long trainingEntryIndex = 0; trainingEntryIndex < numberOfWindows; ++trainingEntryIndex)
        {
            ...
            if (tick == maxNumberOfTicks)
            {
                if (windowTrace)
                {
                    windowTrace[trainingEntryIndex] = 255;
                }
            ...
            if (windowTrace)
            {
                windowTrace[trainingEntryIndex] = predicted;
            }
    ...
    unsigned int initializeANN(unsigned char* publicKey, unsigned char* nonce)
    {
        ...
        random2(rootHash, pool, (unsigned char*)&initValue.lutInit, sizeof(initValue.lutInit));
        ...
        random2(searchHash, pool, (unsigned char*)&initValue.mutationSeed, sizeof(initValue.mutationSeed));
